...
var require_fetch_node = __commonJS({
  "node_modules/fetch-ponyfill/fetch-node.js"(exports, module2) {
    "use strict";
    var fetch2 = require_lib2();
    function wrapFetchForNode(fetch3) {
      return function(u, options) {
        if (typeof u === "string" && u.slice(0, 2) === "//") {
          return fetch3("https:" + u, options);
        }
        return fetch3(u, options);
      };
    }
    module2.exports = function(context) {
      var fetchFn = fetch2.default ? fetch2.default : fetch2;
...
var todoParserRegex = /(\s*)\[([ xX~])\](.+)$/;
var parseLine = (line) => {
  const parseResult = todoParserRegex.exec(line);
  if (!parseResult)
    return null;
  ...
  if (status && trimmedName) {
    const todo = { status, title: trimmedName, scope: whitespaces.length };
    return todo;
  }
  return null;
};
var findTodoList = async (filepath) => {
  const data = await import_promises.default.readFile(filepath, { encoding: "utf-8" });
  const todos = [];
...
// sync.ts
var import_fetch_ponyfill = __toESM(require_fetch_node());
var import_node_querystring = __toESM(require("node:querystring"));
var dotenv = __toESM(require_main());
var import_prompts = __toESM(require_prompts3());
var { fetch } = (0, import_fetch_ponyfill.default)();
dotenv.config();
var { TRELLO_API_KEY } = process.env;
var { TRELLO_API_TOKEN } = process.env;
var { TRELLO_BOARD_ID } = process.env;
var { LABEL_FIRST_LAYER } = process.env;
var { TODO_SEPARATOR = "-" } = process.env;
var getWholeTitleFromTodo = (todo) => {
  if (!todo.parent)
    return [todo.title];
  return [...getWholeTitleFromTodo(todo.parent), todo.title];
};
var processTodos = (_todos) => {
  const labels = LABEL_FIRST_LAYER ? new Set(_todos.filter((todo) => !todo.parent)) : /* @__PURE__ */ new Set();
...
var queryString = (query) => {
  return import_node_querystring.default.encode(query);
};
var authQuery = { key: TRELLO_API_KEY || "", token: TRELLO_API_TOKEN || "" };
var boardEndpoint = (root, query = authQuery, ...resources) => `https://api.trello.com/1/${root}/${TRELLO_BOARD_ID}/${resources.join("/")}?${queryString(query)}`;
var rootEndpoint = (root, query = authQuery, ...resources) => `https://api.trello.com/1/${root}/${resources.join("/")}?${queryString(query)}`;
var mainBoardEndpoint = (query = authQuery, ...resources) => boardEndpoint("board", query, ...resources);
var listsEndpoint = (query = authQuery) => boardEndpoint("board", query, "lists");
var cardsEndpoint = (query = authQuery) => boardEndpoint("board", query, "cards");
var cardEndpoint = (cardId, query = authQuery) => rootEndpoint("cards", query, cardId);
var cardLabelEndpoint = (cardId, query = authQuery) => rootEndpoint("cards", query, cardId, "idLabels");
var fetchLists = async () => {
  const req = await fetch(listsEndpoint());
  const json = await req.json();
  return json;
};
var categorizeLists = (lists) => {
...
var getBoardSettings = async () => {
  const req = await fetch(mainBoardEndpoint());
  const json = await req.json();
  return json;
};
var wait = (duration) => new Promise((res, rej) => {
...
var syncLabels = async (labels, labelNames) => {
  if (labels.size <= 0)
    return [];
  const currentTrelloLabels = new Map((await getLabels()).filter((label) => label.name).map((label) => [label.name, label]));
  ...
  return [allLabels, labelsToSyncToTodoList];
};
var createLabel = async (name, color) => {
...
var diffTodos = async (todos, labels, cards, statusMap2) => {
  const todosToUpload = /* @__PURE__ */ new Set();
  const sameTodos = /* @__PURE__ */ new Set();
  const todosToUpdateLists = /* @__PURE__ */ new Set();
  const todosToUpdateLabels = /* @__PURE__ */ new Set();
  ...
  for (const todo of todos) {
    ...
    if (found)
      sameTodos.add([todo, found]);
    else
      todosToUpload.add(todo);
  }
  for (const todoPair of sameTodos) {
    const [todo, trello] = todoPair;
    const correctList = statusMap2[todo.status];
  ...
  for (const todoPair of sameTodos) {
    ...
    if (todoLabel) {
      ...
      if (!hasTitle)
        todosToUpdateLabels.add(todoPair);
    }
  }
  return ret;
};
var updateList = async (todo, card, statusMap2) => {
  const correctList = statusMap2[todo.status];
  return await (await fetch(cardEndpoint(card.id, { ...authQuery, idList: correctList.id }), {
    method: "PUT"
  })).json();
};
var updateLabel = async (todo, card, labelMap) => {
  if (!todo.label || !labelMap.get(todo.label))
    throw new Error(`Could not find valid label to apply to todo "${todo.title}"`);
  const correctLabel = labelMap.get(todo.label);
  return await (await fetch(cardLabelEndpoint(card.id, { ...authQuery, value: correctLabel }), {
    method: "POST"
  })).json();
};
var createCard = async (todo, labelMap, statusMap2) => {
...
var updateLists = async (todosToUpdateLists, statusMap2) => {
  return await Promise.all([...todosToUpdateLists].map(([todo, card]) => updateList(todo, card, statusMap2)));
};
var updateLabels = async (todosToUpdateLabels, labelMap) => {
  return await Promise.all([...todosToUpdateLabels].map(([todo, card]) => updateLabel(todo, card, labelMap)));
};
var createCards = async (todosToAdd, labelMap, statusMap2) => {
  return await Promise.all([...todosToAdd].map((todo) => createCard(todo, labelMap, statusMap2)));
};
var categorizeLabels = (currentTrelloLabels) => {
  return new Map(currentTrelloLabels.map((e) => [e.name, e.id]));
};
var sync = async (_todos) => {
  if (!TRELLO_API_KEY) {
  ...
  try {
    ...
    const statusMap2 = categorizeLists(lists);
    const inverseStatusMap = new Map(Object.entries(statusMap2).map(([status, card]) => [card.id, status]));
    const cards = await fetchCards();
    const wipTodos = await diffTodos(todos, labels, cards, statusMap2);
    if (wipTodos.todosToUpdateLists.size > 0) {
      ...
      if (value) {
        ...
        await updateLists(wipTodos.todosToUpdateLists, statusMap2);
      }
    }
    if (wipTodos.todosToUpdateLabels.size > 0) {
      console.log("Cards to update labels");
      wipTodos.todosToUpdateLabels.forEach(([todo, card]) => {
        console.log(`"${todo.title}" ${JSON.stringify(card.labels.map((e) => e.name))}-> ${JSON.stringify(card.labels.concat({ name: todo.label || "Empty Label" }).map((e) => e.name))}`);
      });
      const { value } = await (0, import_prompts.default)({
        type: "confirm",
        name: "value",
    ...
    if (wipTodos.todosToUpload.size > 0) {
      ...
      if (value) {
        ...
        const results = await createCards(wipTodos.todosToUpload, labelMap, statusMap2);
        console.log(results);
        const errors = results.map((res, i) => [res, [...wipTodos.todosToUpload][i]]).filter(([res]) => res.error);
...
// index.ts
var asyncGlob = (search) => {
  return new Promise((resolve, reject) => {
    (0, import_glob.default)(search, async (err, matches) => {
      if (err)
        reject(err);
      else
        resolve(matches);
